...
// Load config and helpers ONCE, before any output
...
// Ensure an uploads directory exists next to this file (admin/products/uploads)
...
/* -------------------------------------------------------
   Fallback helpers (only used if not present in functions.php)
-------------------------------------------------------- */
...
if (!function_exists('fetch_attributes_by_category')) {
    function fetch_attributes_by_category(PDO $pdo, int $category_id): array {
        // Returns arrays keyed by attribute type. Adjust to match your schema.
        ...
        $sql = "
            ...
            ORDER BY a.type, a.value
        ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$category_id]);
        while ($row = $stmt->fetch()) {
            if (isset($out[$row['type']])) {
                $out[$row['type']][] = ['id' => (int)$row['id'], 'value' => $row['value']];
            }
        }
        return $out;
    }
...
if (!function_exists('get_currencies')) {
    function get_currencies(PDO $pdo): array {
        // Expected table: currencies(code VARCHAR PK, is_base TINYINT)
        $rows = $pdo->query("SELECT code, is_base FROM currencies ORDER BY code")->fetchAll();
        $base = 'NGN';
        foreach ($rows as $r) {
            if (!empty($r['is_base'])) { $base = $r['code']; break; }
        }
...
if (!function_exists('upload_image')) {
    function upload_image(?array $file, string $destDir): ?string {
        ...
        if ($finfo === false) throw new RuntimeException('File is not an image.');

        $ext = image_type_to_extension($finfo[2], true) ?: '.jpg';
        ...
        // Return relative path from project root (adjust if different)
        $rel = str_replace(BASE_PATH . DIRECTORY_SEPARATOR, '', $target);
        $rel = str_replace('\\', '/', $rel);
        return $rel; // e.g. "admin/products/uploads/abc123.jpg"
    }
}
...
$allowedAttrs = $currentCatId
    ? fetch_attributes_by_category($pdo, $currentCatId)
    : ['occasion'=>[], 'length'=>[], 'style'=>[]];

/* -------------------------------------------------------
   Handle POST (create product + variants + attributes)
-------------------------------------------------------- */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image_path = null;
    try {
        $image_path = upload_image($_FILES['image_file'] ?? null, $uploadDir);
    ...
    if (!$errors) {
        ...
        // insert product
        $stmt = $pdo->prepare("
            INSERT INTO products (category_id, name, slug, sku, description, base_currency_code, base_price, image_path)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        ...
        if ($attrIds) {
            ...
            ");
            $check->execute(array_merge([$category_id], $attrIds));
        ...
        if (!empty($_POST['size_rows']) && is_array($_POST['size_rows'])) {
            $ins = $pdo->prepare("
                INSERT INTO product_variants (product_id, size, color, price_override, stock, image_path)
                VALUES (?, ?, NULL, ?, ?, ?)
            ");
            foreach ($_POST['size_rows'] as $i => $row) {
                ...
                if ($size !== '' && $price !== '' && is_numeric($price)) {
                    ...
                    if ($vImg && ($vImg['error'] === UPLOAD_ERR_OK)) {
                        try { $vPath = upload_image($vImg, $uploadDir); } catch (RuntimeException $e) { $errors[] = $e->getMessage(); }
                    }
        ...
        if (!$errors) {
            $ok = true;
            // reload allowed attributes for the current category
            $allowedAttrs = $category_id
                ? fetch_attributes_by_category($pdo, (int)$category_id)
                : ['occasion'=>[], 'length'=>[], 'style'=>[]];
...
<div class="page-wrapper compact-wrapper" id="pageWrapper">
  ...
  <div class="page-body-wrapper">
    ...
    <div class="page-body ">
      ...
      <?php if ($ok): ?>
        <div class="alert container bg-success border-0 shadow-sm rounded-3">
          <strong>Saved.</strong> Product saved successfully.
        </div>
      <?php endif; ?>

      <?php if ($errors): ?>
        <div class="alert container bg-danger border-0 shadow-sm rounded-3">
          <strong>Please fix:</strong>
          <ul class="mb-0"><?php foreach ($errors as $e) echo "<li>".htmlspecialchars($e)."</li>"; ?></ul>
        </div>
      <?php endif; ?>

      <form method="post" enctype="multipart/form-data" class="needs-validation container" novalidate>
        <div class="row g-4">
          <!-- Left -->
          <div class="col-lg-8 ">
            <div class="card">
                ...
                <div class="col-md-3">
                  ...
                  <select name="base_currency_code" class="form-select" required>
                    <?php foreach ($currencies as $cur): ?>
                      <option value="<?= htmlspecialchars($cur['code']) ?>" <?= !empty($cur['is_base']) ? 'selected' : '' ?>>
            ...
            <div class="d-flex gap-3 page-actions">
              <button class="btn bg-primary px-4" type="submit">Save Product</button>
              <a href="admin-dashboard.php" class="btn btn-ghost px-4">Cancel</a>
            </div>
          </div>
...
function inputEl(opts={}){ const el=document.createElement('input'); el.className='form-control'; Object.assign(el,opts); return el; }
...
function addSizeRow() {
  ...
  const size  = inputEl({ name: `size_rows[${sizeIdx}][size]`,  placeholder: 'e.g. S, M, L' });
  const price = inputEl({ name: `size_rows[${sizeIdx}][price]`, type:'number', step:'0.01', placeholder:'0.00' });
  const stock = inputEl({ name: `size_rows[${sizeIdx}][stock]`, type:'number', step:'1', value:'0' });
...
function addColorRow() {
  ...
  const color = inputEl({ name: `color_rows[${colorIdx}][color]`, placeholder: 'e.g. Black, Red' });
  const price = inputEl({ name: `color_rows[${colorIdx}][price]`, type:'number', step:'0.01', placeholder:'0.00' });
  const stock = inputEl({ name: `color_rows[${colorIdx}][stock]`, type:'number', step:'1', value:'0' });
...
function addComboRow() {
  ...
  const size  = inputEl({ name: `combo_rows[${comboIdx}][size]`,  placeholder: 'e.g. L' });
  const color = inputEl({ name: `combo_rows[${comboIdx}][color]`, placeholder: 'e.g. Black' });
  const price = inputEl({ name: `combo_rows[${comboIdx}][price]`, type:'number', step:'0.01', placeholder:'0.00' });
  const stock = inputEl({ name: `combo_rows[${comboIdx}][stock]`, type:'number', step:'1', value:'0' });
...
<?php include __DIR__ . '/../includes/logout.php'; ?>
<?php include __DIR__ . '/../includes/script-js.php'; ?>
