 ...
 * - $menu: array of categories with allowed attributes (occasion/length/style)
 * - $display: current currency code
 *
 * If not provided by the parent, we’ll build safe defaults.
 */
if (!isset($menu) || !is_array($menu)) {
    if (!function_exists('fetch_categories')) {
        require_once __DIR__ . '/includes/functions.php';
    }
    if (function_exists('fetch_categories') && isset($pdo)) {
        $cats = fetch_categories($pdo);
        $menu = [];
        foreach ($cats as $cat) {
            if (!function_exists('fetch_attributes_by_category')) break;
            $allowed = fetch_attributes_by_category($pdo, (int)$cat['id']);
            $menu[] = [
                'id'      => $cat['id'],
                'name'    => $cat['name'],
                'slug'    => $cat['slug'],
                'allowed' => $allowed ?? ['occasion'=>[], 'length'=>[], 'style'=>[]],
            ];
        }
    } else {
        $menu = []; // fallback empty
...
if (!isset($display)) {
    // Try to get currencies so we can default display
    if (!function_exists('get_currencies')) {
        require_once __DIR__ . '/functions.php';
    }
    $display = 'NGN';
    if (function_exists('get_currencies') && isset($pdo)) {
        [$currencies, $baseCode] = get_currencies($pdo);
        $display = $baseCode ?: 'NGN';
    }
}
?>
<style>
    ...
    <span>
      <?php foreach ($menu as $m): ?>
        <div class="dropdown category-item">
          <a href="<?= BASE_URL ?>shop/shop.php?cat=<?= urlencode($m['slug']) ?>&cur=<?= urlencode($display) ?>">
            <?= htmlspecialchars($m['name']) ?>
          </a>
          <!-- Hover menu with allowed attributes -->
          <div class="menu">
            <?php if (!empty($m['allowed']['occasion'])): ?>
              <h4>Occasion</h4>
              <?php foreach ($m['allowed']['occasion'] as $o): ?>
                <a href="<?= BASE_URL ?>shop/shop.php?cat=<?= urlencode($m['slug']) ?>&occasion=<?= urlencode($o['value']) ?>&cur=<?= urlencode($display) ?>">
                  <?= htmlspecialchars($o['value']) ?>
                </a>
              <?php endforeach; ?>
            <?php endif; ?>

            <?php if (!empty($m['allowed']['length'])): ?>
              <h4>Length</h4>
              <?php foreach ($m['allowed']['length'] as $l): ?>
                <a href="<?= BASE_URL ?>shop/shop.php?cat=<?= urlencode($m['slug']) ?>&length=<?= urlencode($l['value']) ?>&cur=<?= urlencode($display) ?>">
                  <?= htmlspecialchars($l['value']) ?>
                </a>
              <?php endforeach; ?>
            <?php endif; ?>

            <?php if (!empty($m['allowed']['style'])): ?>
              <h4>Style</h4>
              <?php foreach ($m['allowed']['style'] as $s): ?>
                <a href="<?= BASE_URL ?>shop/shop.php?cat=<?= urlencode($m['slug']) ?>&style=<?= urlencode($s['value']) ?>&cur=<?= urlencode($display) ?>">
